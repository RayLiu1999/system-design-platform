...
const topicContent = {
  'cap-theorem': {
    concepts: [
      { title: 'CAP Theorem 核心', text: 'CAP Theorem 指出分散式系統無法同時滿足一致性（Consistency）、可用性（Availability）和分區容錯（Partition Tolerance）三者。由於網路分區在分散式環境中不可避免，實際上我們只能在 CP 和 AP 之間做取捨。' },
      { title: 'CP vs AP 選擇', text: 'CP 系統（如 ZooKeeper、etcd）在分區時拒絕不一致的請求，保證資料正確性。AP 系統（如 Cassandra、DynamoDB）在分區時繼續服務，但可能回傳過時資料。選擇取決於業務：金融交易選 CP，社交動態選 AP。' },
      { title: 'ACID vs BASE', text: 'ACID（Atomicity, Consistency, Isolation, Durability）是傳統關聯式資料庫的事務保證。BASE（Basically Available, Soft state, Eventually consistent）是 NoSQL 的設計哲學，犧牲強一致性換取高擴展性。' },
      { title: 'Eventual Consistency', text: '最終一致性意味著在沒有新寫入的情況下，所有副本最終會收斂到相同狀態。實務上需要搭配版本向量（Version Vector）、衝突解決策略（Last-Write-Wins, CRDT）來處理並發更新。' },
    ],
    interview: [
      { question: '解釋 CAP Theorem，為什麼不能三者兼得？', answer: 'CAP Theorem 證明了在網路分區（P）發生時，系統必須在一致性（C）和可用性（A）之間做選擇。當兩個節點間網路中斷，要麼拒絕請求保證一致性（CP），要麼繼續服務但可能回傳舊資料（AP）。P 是必選的，因為網路分區無法避免。', keywords: ['CAP', 'Partition Tolerance', 'CP vs AP'] },
      { question: 'ACID 和 BASE 的差異？各適用什麼場景？', answer: 'ACID 提供強事務保證，適合銀行轉帳等不容許不一致的場景。BASE 提供最終一致性，適合社交媒體、電商商品瀏覽等可容忍短暫不一致的高流量場景。選擇依據是業務對一致性的容忍度和對擴展性的需求。', keywords: ['ACID', 'BASE', 'Eventual Consistency'] },
      { question: '什麼是 Eventual Consistency？如何處理衝突？', answer: '最終一致性保證在無新寫入後，所有副本終將一致。衝突解決策略包括：Last-Write-Wins（用時間戳選最新）、向量時鐘（偵測因果關係）、CRDT（數學保證無衝突合併）。Dynamo 使用向量時鐘，Redis 使用 LWW。', keywords: ['Vector Clock', 'LWW', 'CRDT'] },
    ],
  },
  'scalability': {
    concepts: [
      { title: '擴展性設計原則', text: '可擴展性是指系統在負載增加時，透過增加資源來維持效能的能力。垂直擴展（Scale Up）增加單機硬體，簡單但有上限；水平擴展（Scale Out）增加伺服器數量，理論上無上限但需處理分散式複雜性。' },
      { title: '水平擴展的挑戰', text: '水平擴展需要解決：狀態管理（Session 需外部化到 Redis）、資料分片（決定 Sharding 策略）、服務發現（動態註冊新節點）、一致性（跨節點資料同步）。無狀態（Stateless）設計是水平擴展的先決條件。' },
      { title: '擴展性模式', text: '常見模式包括：讀寫分離（Read Replica）、CQRS（命令查詢責任分離）、微服務解耦、非同步處理（Message Queue）、快取層（CDN + Redis）。每種模式解決特定瓶頸，需根據 profiling 結果選擇。' },
    ],
    interview: [
      { question: '垂直擴展和水平擴展的差異？', answer: '垂直擴展（Scale Up）是升級單機 CPU/RAM/SSD，優點是簡單、無分散式問題；缺點是硬體上限且單點故障。水平擴展（Scale Out）是增加機器數量，優點是理論上無上限、有容錯能力；缺點是需處理資料一致性、Session 管理等分散式問題。通常先垂直後水平。', keywords: ['Scale Up', 'Scale Out', 'Stateless'] },
      { question: '如何設計一個可水平擴展的系統？', answer: '關鍵原則：(1) 無狀態設計，Session 存 Redis；(2) 資料庫讀寫分離 + Sharding；(3) 用 Message Queue 解耦服務；(4) 有效的快取策略；(5) 服務發現 + 負載均衡。目標是讓每個組件都可以獨立水平擴展。', keywords: ['Stateless', 'Sharding', 'Cache', 'MQ'] },
      { question: '什麼是 CQRS？為什麼它有助於擴展？', answer: 'CQRS 將讀取和寫入操作分開處理。寫入走主庫保證一致性，讀取走多個 Read Replica 分散壓力。讀寫比通常 10:1 甚至更高，分離後讀取端可以獨立擴展、使用不同的資料模型（例如 Elasticsearch）優化查詢。', keywords: ['CQRS', 'Read Replica', '讀寫分離'] },
    ],
  },
  'high-availability': {
    concepts: [
      { title: '高可用性定義', text: '高可用性（HA）是指系統在部分組件故障時仍能持續提供服務的能力。通常用 SLA（Service Level Agreement）衡量：99.9%（年停機 8.76 小時）、99.99%（52.6 分鐘）、99.999%（5.26 分鐘）。' },
      { title: '容錯移轉（Failover）', text: 'Active-Standby：一主一備或一主多備，主節點故障時自動切換。Active-Active：所有節點同時處理流量，故障節點的流量自動分配到其他節點。Active-Active 可用性更高但一致性更難保證。' },
      { title: 'Health Check 機制', text: 'Kubernetes 使用 Liveness Probe（檢測進程是否存活，失敗則重啟）和 Readiness Probe（檢測是否準備接收流量，失敗則從 Service 摘除）。合理設計探針的 initialDelaySeconds、periodSeconds 和 failureThreshold 至關重要。' },
    ],
    interview: [
      { question: '如何設計一個高可用性系統？', answer: '(1) 消除單點故障（SPOF）：資料庫主從、多可用區部署；(2) 故障偵測：Health Check + 心跳機制；(3) 自動容錯移轉：DNS Failover、Load Balancer 摘除故障節點；(4) 資料冗餘：跨區域 Replication；(5) 優雅降級：Circuit Breaker + Fallback。', keywords: ['SPOF', 'Failover', 'Replication', 'Circuit Breaker'] },
      { question: 'Active-Standby 和 Active-Active 的差異？', answer: 'Active-Standby：簡單，備機僅在主機故障時接管，但切換有延遲（RTO）且備機平時閒置。Active-Active：所有節點同時服務，更高吞吐量和可用性，但需處理資料同步和衝突，如跨 DC 寫入問題。選擇取決於 RTO/RPO 要求和成本預算。', keywords: ['Active-Standby', 'Active-Active', 'RTO', 'RPO'] },
      { question: '什麼是 SLA？如何計算系統可用性？', answer: '可用性 = (總時間 - 停機時間) / 總時間。多組件串聯的整體可用性是各組件可用性的乘積：99.9% × 99.9% = 99.8%。提升策略：增加冗餘（並聯提升可用性）、減少 MTTR（平均修復時間）、增加 MTBF（平均故障間隔時間）。', keywords: ['SLA', 'MTTR', 'MTBF', '冗餘設計'] },
    ],
  },
  'api-design': {
    concepts: [
      { title: 'REST vs gRPC vs GraphQL', text: 'REST 基於 HTTP/JSON，簡單通用，適合對外 API。gRPC 基於 HTTP/2 + Protobuf，高效能低延遲，適合內部微服務通訊。GraphQL 讓客戶端精確查詢所需欄位，解決 Over-fetching / Under-fetching，適合複雜前端需求。' },
      { title: 'API 版本管理', text: '常見策略：URL 路徑版本（/v1/users）、Header 版本（Accept: application/vnd.api.v1+json）、Query Parameter（?version=1）。URL 路徑最直觀且容易快取，是業界最常用方式。向後相容設計可延長 API 壽命。' },
      { title: '冪等性（Idempotency）', text: '冪等操作多次執行結果相同。GET、PUT、DELETE 天然冪等；POST 需額外設計。常用方式：客戶端帶 Idempotency-Key，伺服端在 Redis 中記錄已處理的 Key，避免重複處理。支付場景中冪等性尤其重要。' },
    ],
    interview: [
      { question: 'REST、gRPC、GraphQL 如何選型？', answer: 'REST：對外公開 API、Browser 直接呼叫、需要 HTTP 快取。gRPC：微服務間高頻通訊、需要雙向 streaming、對延遲敏感。GraphQL：前端需求多變、需要彈性查詢、想減少 API 數量。大型系統通常混用：對外 REST/GraphQL，內部 gRPC。', keywords: ['REST', 'gRPC', 'GraphQL', 'Protobuf'] },
      { question: '如何設計冪等 API？', answer: '(1) 客戶端生成唯一 Idempotency-Key 放在 Header 中；(2) 伺服端在 Redis 中檢查 Key 是否已處理；(3) 若已處理回傳快取結果，未處理則執行並存入 Redis；(4) 設定合理的 Key TTL（通常 24 小時）。Stripe 支付 API 就是最佳實踐範例。', keywords: ['Idempotency-Key', 'Redis', 'At-Least-Once'] },
      { question: 'API Rate Limiting 的實作方式？', answer: '常見算法：Token Bucket（平滑限流、允許突發）、Sliding Window（精確統計）、Fixed Window（簡單但有邊界問題）。實作位置：API Gateway 層面統一攔截。回應 429 Too Many Requests 並加上 Retry-After 和 X-RateLimit-Remaining Header。', keywords: ['Token Bucket', 'Sliding Window', '429'] },
    ],
  },
  'load-balancing': {
    concepts: [
      { title: 'L4 vs L7 負載均衡', text: 'L4（傳輸層）基於 IP/Port 轉發，效能高、無法感知應用層內容。L7（應用層）基於 HTTP Header/URL/Cookie 路由，可做內容分發、A/B Testing、認證檢查，但效能略低。Nginx、HAProxy 支援兩者，AWS ALB 是 L7，NLB 是 L4。' },
      { title: '負載均衡算法', text: 'Round Robin：輪詢，簡單公平。Weighted Round Robin：按權重分配，考慮伺服器效能差異。Least Connections：分配給當前連線數最少的伺服器，適合長連接場景。IP Hash：同一 IP 固定到同一伺服器，用於 Session 親和性。' },
      { title: '健康檢查與故障處理', text: '主動健康檢查：LB 定期向後端發 HTTP 請求，連續失敗則摘除節點。被動健康檢查：監測實際請求的失敗率。優雅降級（Graceful Degradation）：節點標記為 draining 後不再接新連線，等舊連線結束再下線。' },
    ],
    interview: [
      { question: 'L4 和 L7 負載均衡的差異？', answer: 'L4 工作在 TCP/UDP 層，根據 IP 和 Port 轉發，速度快但智商低。L7 工作在 HTTP 層，能根據 URL、Header、Cookie 做路由決策，支援 SSL 終止、壓縮、快取。選擇依據：純粹的流量分發用 L4，需要請求內容感知用 L7。', keywords: ['L4', 'L7', 'SSL Termination', 'Content-Based Routing'] },
      { question: '如何處理 Session 一致性問題？', answer: '方案：(1) Sticky Session（IP Hash）：簡單但不均衡；(2) Session 外部化到 Redis：推薦，伺服器完全無狀態；(3) JWT Token：無需 Session，但 Token 大小和撤銷是問題。最佳實踐是 Session 外部化 + Stateless 架構。', keywords: ['Sticky Session', 'Redis Session', 'JWT', 'Stateless'] },
      { question: 'DNS 負載均衡的優缺點？', answer: '優點：全球級別分發、無單點故障、對用戶透明。缺點：DNS TTL 導致切換慢（分鐘級）、無法做健康檢查、無法感知伺服器負載。通常作為第一層全球流量導向（GeoDNS），搭配 L4/L7 LB 做細粒度分發。', keywords: ['GeoDNS', 'TTL', '多層負載均衡'] },
    ],
  },
  'cdn-proxy': {
    concepts: [
      { title: 'CDN 原理', text: 'CDN（Content Delivery Network）在全球部署邊緣節點，將靜態內容（圖片、CSS、JS）快取到離用戶最近的節點。用戶請求先到 CDN 邊緣，命中快取直接回傳（Cache HIT），未命中則回源（Origin Pull）後快取。可將延遲從數百毫秒降至數十毫秒。' },
      { title: '反向代理（Reverse Proxy）', text: '反向代理接收客戶端請求並轉發到後端伺服器。核心價值：SSL 終止、壓縮、請求緩衝、靜態檔案快取、安全屏障。Nginx 是最常見的反向代理，配合 upstream 模組做負載均衡。正向代理代表客戶端，反向代理代表伺服器。' },
      { title: 'Cache-Control 策略', text: 'HTTP 快取控制：max-age（快取存活時間）、no-cache（每次需驗證）、no-store（不快取）、private/public（區分用戶私有和公共內容）。CDN 層面還有 s-maxage（CDN 專用 TTL）和 stale-while-revalidate（過期時先回傳舊資料再更新）。' },
    ],
    interview: [
      { question: 'CDN 的工作原理和使用場景？', answer: '原理：用戶 DNS 解析到最近的 CDN 邊緣節點 → 邊緣檢查快取 → HIT 直接回傳 / MISS 回源拉取並快取。場景：靜態資源加速（圖片、影片、JS/CSS）、全球化應用降低延遲、軟體分發、DDoS 防護（吸收流量）。選型：CloudFlare（免費方案好）、CloudFront（與 AWS 整合好）。', keywords: ['Edge Node', 'Origin Pull', 'Cache HIT/MISS'] },
      { question: '如何設計 CDN 快取失效策略？', answer: '(1) 基於 URL 版本化：style.v2.css，URL 變則快取自動失效；(2) Cache-Control Header：設定合理 max-age 和 s-maxage；(3) CDN API 主動 Purge：內容更新時呼叫 CDN 清除舊快取；(4) stale-while-revalidate：舊版先用、背景更新，避免回源風暴。', keywords: ['Cache Invalidation', 'Purge', 'Versioning'] },
      { question: '正向代理和反向代理的差異？', answer: '正向代理（Forward Proxy）代表客戶端，隱藏客戶端身份，用於翻牆、內容過濾。反向代理（Reverse Proxy）代表伺服器，隱藏後端架構，用於負載均衡、SSL 終止、快取。企業架構中反向代理更常見，Nginx 是業界標準。', keywords: ['Forward Proxy', 'Reverse Proxy', 'Nginx', 'SSL Termination'] },
    ],
  },
  'relational-db': {
    concepts: [
